...
class FMModelForm_maker_fmc {
  ...
  public function check_ip($id) {
    ...
    if ( $blocked_ip ) {
      $_SESSION[ 'massage_after_submit' . $id ] = addslashes( __( 'Tu IP se encuentra bloqueada. Por favor, pongase en contacto con el administrador de la web.', WDFMInstance(self::PLUGIN)->prefix ) );
    ...
    foreach ( $label_type as $key => $type ) {
      ...
      if ( !in_array( $i, $disabled_fields ) ) {
        ...
        if ( $invalid_email_address ) {
          return array( 'error' => true, 'group_id' => $group_id, 'message' => addslashes( addslashes( __( 'Introduzca una dirección de correo válida', WDFMInstance(self::PLUGIN)->prefix ) ) ) );
        }
  ...
  public function gen_mail( $group_id = 0, $all_files = array(), $id = 0, $str = '' ) {
    ...
    $_SESSION[ 'error_or_no' . $id ] = 0;
    $msg = addslashes( __( 'Se ha enviado el formulario correctamente', WDFMInstance(self::PLUGIN)->prefix ) );
    if ( $row->sendemail ) {
      if ( $row->mail || $row->send_to ) {
        if ( $send ) {
          if ( $send !== TRUE ) {
            $_SESSION[ 'error_or_no' . $id ] = 1;
            $msg = addslashes( __( 'Error, El email no se envió.', WDFMInstance(self::PLUGIN)->prefix ) );
          }
          else {
            $_SESSION[ 'error_or_no' . $id ] = 0;
          }
