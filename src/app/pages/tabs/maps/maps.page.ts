...
export class MapsPage implements OnInit, AfterContentInit {
  ...
  showMap() {
    ...
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      this.map = new google.maps.Map(
        this.mapElement.nativeElement,
        {
          center: pos,
          zoom: 8,
          // Desactivamos el modo Street View
      ...
      if (this.sportCenters.length != 0) {
        // Añadimos las marcas de posición de los lugares
        this.addMarkersToMap(this.sportCenters);   
      }
    }).catch(async (e) => {
      switch (e.code) {
    ...
    if (this.sportCenters.length == 0 && this.sport != null) {
      ...
      this.sweetAlertService.showAlert(this.env.titleErrorSportcentersMaps, this.env.errorSportcenterMaps, "error");
  ...
  // Metodo para cerrar las ventanas de informacion del Google Maps
  closeAllInfoWindows() {
